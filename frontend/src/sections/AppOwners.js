...
import Table from '../components/Owner/Table';
...
const App = () => {
  ...
  const ownersEndpoint = window.__APP_DATA__.ownersEndpoint;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const newData = await fetchApi(ownersEndpoint, {method: 'GET'}, config.API_TIMEOUT);
